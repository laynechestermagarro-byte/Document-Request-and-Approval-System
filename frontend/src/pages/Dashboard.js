    ...
    <div className="flex items-center justify-between p-6 rounded-2xl border border-slate-100 hover:bg-slate-50/50 transition-colors">
      <div className="flex items-center gap-5">
        ...
        <div>
          ...
          {description && <p className="text-slate-500 text-xs mt-2 italic bg-slate-50 px-3 py-1 rounded-lg inline-block">“{description}”</p>}
        </div>
      </div>
      <div className="flex items-center gap-4">
        <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border ${styles[status] || 'bg-slate-50'}`}>{status}</span>
        {isAdmin && status === "Under Review" && (
          <div className="flex gap-2">
            <button onClick={onApprove} className="bg-emerald-600 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-sm">Approve</button>
            <button onClick={onReject} className="bg-red-500 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-sm">Reject</button>
          </div>
        )}
      </div>
    </div>
...
const Dashboard = () => {
  ...
  // Solves the "Missing Dependency" warning
  ...
  const handleStatusUpdate = async (docId, newStatus) => {
    try {
      await axios.patch(`http://localhost:5000/api/docs/status/${docId}`, { status: newStatus });
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                requests.map(req => (
                  <RequestItem 
                    key={req._id} 
                    name={userRole === 'Admin' ? `${req.requester?.name || 'User'} - ${req.documentType}` : req.documentType} 
